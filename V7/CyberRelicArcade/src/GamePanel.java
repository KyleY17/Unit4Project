...
public class GamePanel extends JPanel implements ActionListener, KeyListener {
    ...
    // ── Shop Items ────────────────────────────────────────────────────────
    // Define shop items with their ID, name, cost, and description
    static final String SHOP_ITEM_EXTRA_LIFE = "extra_life";
    static final String SHOP_ITEM_SHIELD = "shield";
    static final long COST_EXTRA_LIFE = 2500;
    static final long COST_SHIELD = 5000;
    
    // Shop item data structure: ID -> {name, cost, description}
    static final Map<String, Object[]> SHOP_ITEMS = new HashMap<String, Object[]>() {{
        put(SHOP_ITEM_EXTRA_LIFE, new Object[]{"EXTRA LIFE", COST_EXTRA_LIFE, "Gain +1 life (max 4)", 0});
        put(SHOP_ITEM_SHIELD, new Object[]{"SHIELD", COST_SHIELD, "Start next game with shield", 1});
    }};
    ...
    void newGame() {
        ...
        // Apply purchased shop items
        if (loggedInUsername != null) {
            Set<String> purchasedItems = userManager.getPurchasedItems(loggedInUsername);
            
            // Extra life purchased: start with 4 lives instead of 3
            if (purchasedItems.contains(SHOP_ITEM_EXTRA_LIFE)) {
                lives = 4;
            }
            
            // Shield purchased: start with shield active
            if (purchasedItems.contains(SHOP_ITEM_SHIELD)) {
                hasShield = true;
            }
        }
    ...
    protected void paintComponent(Graphics g0) {
        ...
        switch (state) {
            ...
            case MODE_SELECT -> drawModeSelect(g);
    ...
    void drawShop(Graphics2D g) {
        ...
        if (loggedInUsername != null) {
            purchasedItems = userManager.getPurchasedItems(loggedInUsername);
        }

        // Draw shop items
        Font itemFont = new Font("Courier New", Font.BOLD, 18);
        Font descFont = new Font("Courier New", Font.PLAIN, 14);
        g.setFont(itemFont);
        ...
        // Item 1: Extra Life
        drawShopItem(g, itemFont, descFont, 1, "EXTRA LIFE", 2500, 
                     "Gain +1 life", purchasedItems.contains(SHOP_ITEM_EXTRA_LIFE), totalCoins, itemY);
        
        // Item 2: Shield
        drawShopItem(g, itemFont, descFont, 2, "SHIELD", 5000, 
                     "Start with shield", purchasedItems.contains(SHOP_ITEM_SHIELD), totalCoins, itemY + itemSpacing);

        // Instructions
        Font instructFont = new Font("Courier New", Font.BOLD, 14);
        g.setFont(instructFont);
        g.setColor(new Color(100, 180, 255));
        String instruct1 = "Press [ 1 ] or [ 2 ] to purchase   |   [ M ] to return";
        FontMetrics ifm = g.getFontMetrics();
        g.drawString(instruct1, (W - ifm.stringWidth(instruct1)) / 2, 550);
    ...
    /**
     * Helper method to draw a shop item with its details and purchase status.
     */
    void drawShopItem(Graphics2D g, Font itemFont, Font descFont, int itemNumber, String itemName, 
                      long cost, String description, boolean owned, long playerCoins, int y) {
        // Item number (1 or 2)
        g.setFont(itemFont);
        g.setColor(new Color(255, 200, 50));
        g.drawString("[" + itemNumber + "]", 80, y);

        // Item name
        g.setColor(NEON_CYAN);
        g.drawString(itemName, 150, y);

        // Cost or "OWNED" status
        if (owned) {
            g.setColor(NEON_GREEN);
            g.drawString("OWNED", 400, y);
        } else {
            g.setColor(new Color(255, 200, 50));
            g.drawString(cost + " coins", 400, y);
            
            // Show if player can afford it
            if (playerCoins < cost) {
                g.setColor(NEON_RED);
                g.drawString("(need " + (cost - playerCoins) + " more)", 550, y);
            }
        }
        ...
        g.drawString(description, 150, y + 20);
    }

    /**
     * Attempts to purchase a shop item. Checks if the player has enough coins,
     * deducts the cost, and marks the item as purchased.
     */
    void attemptPurchase(String itemId, long cost) {
        if (loggedInUsername == null) {
            return; // Must be logged in to purchase
        }

        long currentCoins = userManager.getCoins(loggedInUsername);
        Set<String> purchasedItems = userManager.getPurchasedItems(loggedInUsername);
        ...
        userManager.spendCoins(loggedInUsername, cost);
        userManager.purchaseItem(loggedInUsername, itemId);
    }
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if (state == State.MODE_SELECT) {
            switch (k) {
                case KeyEvent.VK_LEFT, KeyEvent.VK_A -> modeSelection = 0;  // Singleplayer
                case KeyEvent.VK_RIGHT, KeyEvent.VK_D -> modeSelection = 1;  // Multiplayer
        ...
        // Shop screen: M to back to menu, 1/2 to purchase items
        if (state == State.SHOP) {
            if (k == KeyEvent.VK_1) {
                // Purchase Extra Life
                attemptPurchase(SHOP_ITEM_EXTRA_LIFE, COST_EXTRA_LIFE);
            }
            if (k == KeyEvent.VK_2) {
                // Purchase Shield
                attemptPurchase(SHOP_ITEM_SHIELD, COST_SHIELD);
            }
            if (k == KeyEvent.VK_M) {
                state = State.TITLE;
            }
